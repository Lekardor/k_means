...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    '''
    def get_dis_to_closet_center(index, centers):
        shortest_dis = float("inf")
        for center in centers:
            shortest_dis = min(shortest_dis, np.linalg.norm(x[center] - x[index]))
        return shortest_dis


    centers = np.array(get_lloyd_k_means(len(x), 1, x, generator))
    while centers.shape[0] < n_cluster:
        weights = np.array([get_dis_to_closet_center(index, centers) for index in range(n)])
        total_weights = np.sum(weights)
        weights = weights / total_weights
        round_num = generator.rand()
        total = 0
        index = -1
        while total < round_num:
            index += 1
            total += weights[index]
        centers = np.append(centers, index)
    centers = centers.tolist()
